...
# 시가총액 기준 내림차순 정렬
top100 = top100.sort_values(by='시가총액(억)', ascending=False).reset_index(drop=True)

# 순위 추가
top100.insert(0, '순위', range(1, 101))

# ==================== 테이블 ====================
st.subheader("🔥 코스피 상위 100종목 (시가총액 순)")
...
styled_df = top100[display_cols].style.map(
    color_change, 
    subset=['하루변동률']
...
with col1:
    selected_name = st.selectbox(
        "비교할 종목 선택",
        options=top100['Name'].tolist(),
        index=0
    )
    ticker = top100[top100['Name'] == selected_name]['Code'].iloc[0]
